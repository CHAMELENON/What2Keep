...
from opencood.tools.visualize_heatmap import feature_heatmap
    ...
    def forward(self, batch_confidence_maps, B):
        ...
        for b in range(B):
            ...
            if self.training:
                ...
                ones_fill = torch.ones(L, K, dtype=communication_maps.dtype, device=communication_maps.device)
                communication_mask = torch.scatter(communication_mask, -1, indices, ones_fill).reshape(L, 1, H, W)
            elif self.comm_volume:
                k=int(2**self.comm_volume/4/self.dim)
                communication_mask_list=[]
                for i in range(L):
                    threshold_value = torch.topk(communication_maps[i].view(-1), k, sorted=False).values.min()
                    
                    ones_mask = torch.ones_like(communication_maps[i]).to(communication_maps.device)
                    zeros_mask = torch.zeros_like(communication_maps[i]).to(communication_maps.device)
                    communication_mask_list.append(torch.where(communication_maps[i] > threshold_value, ones_mask, zeros_mask))
            else:
                communication_mask = torch.ones_like(communication_maps).to(communication_maps.device)


            communication_mask=torch.stack(communication_mask_list,dim=0)
            communication_rate = communication_mask.sum() / (L * H * W)
            # Ego
            communication_mask[0] = 1
...
class Where2comm(nn.Module):
    ...
    def forward(self, x, psm_single, record_len, pairwise_t_matrix, backbone=None):
        ...
        if self.multi_scale:
            ...
            for i in range(self.num_levels):
                ...
                if i == 0:
                    if self.fully:
                        ...
                        comm_volume=cal_size(x[1,...]) if (not self.training) and record_len[0]>1 else None
            ...
            x_fuse = torch.stack(x_fuse)
        return x_fuse, communication_rates,comm_volume
