...
def cluster_dpc_knn(x, cluster_num, k=5):
    ...
    with torch.no_grad():
        ...
        # get local density
        #B,N,K
        dist_nearest, index_nearest = torch.topk(dist_matrix, k=k, dim=-1, largest=False) #每个点距离最近的k个点
        #B,N
        #距离越小越好，所以有负号
        density = (-(dist_nearest ** 2).mean(dim=-1)).exp() #每个点距离最近的k个点作平均，平均距离越小，说明密度越大，这是一个递减指数函数
        # print(density.shape)
        ...
        # assign tokens to the nearest center 获得的是所有1600个像素点到300个中心点的距离矩阵
        dist_matrix = index_points(dist_matrix, index_down)
        #获取每个像素点到每个中心类距离最小的索引，即每个像素点属于的类的索引
