...
class SortSampler(nn.Module):
    ...
    def forward(self, src, pos_embed, sample_ratio,dis_priority):
        ...
        sort_order = sample_weight_clone.sort(descending=True,dim=1)[1] #下降的将重要性排序
        sort_confidence_topk = sort_order[:,:max_sample_num] #取前k个重要的特征
        sort_confidence_topk_remaining = sort_order[:,min_sample_num:] #后k个不重要的特征
        # flatten for gathering 将像素展平
        src = src.flatten(2).permute(2, 0, 1)
        src = self.norm_feature(src)
        # 将特征使用置信图筛选出重要的特征,同时会将重要性作为权重作用于特征
