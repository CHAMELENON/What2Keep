...
class IntermediateFusionDataset(basedataset.BaseDataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.visualize:
            ...
            non_delay_projected_lidar_stack = []
        ...
        for cav_id, selected_cav_base in base_data_dict.items():
            ...
            non_delay_processed_features.append(
                selected_cav_processed['non_delay_processed_features'])
            ...
            if self.visualize:
                ...
                non_delay_projected_lidar_stack.append(
                    selected_cav_processed['non_delay_projected_lidar'])
        ...
        non_delay_merged_feature_dict = self.merge_features_to_dict(non_delay_processed_features)
        ...
        processed_data_dict['ego'].update(
            {'object_bbx_center': object_bbx_center,
             ...
             'non_delay_processed_lidar': non_delay_merged_feature_dict,
        ...
        if self.visualize:
            processed_data_dict['ego'].update({'lidar_np':
                np.vstack(
                    projected_lidar_stack)})

            processed_data_dict['ego'].update({'non_delay_lidar_np':
                np.vstack(
                    projected_lidar_stack)})
    ...
    def get_item_single_car(self, selected_cav_base, ego_pose):
        ...
        # 加入无时延的数据：
        # if self.visualize:
        non_delay_lidar_np = selected_cav_base['non_delay_lidar_np']
        non_delay_lidar_np = shuffle_points(non_delay_lidar_np)
        # remove points that hit itself
        non_delay_lidar_np = mask_ego_points(non_delay_lidar_np)
        # project the lidar to ego space
        if self.proj_first:
            non_delay_lidar_np[:, :3] = \
                box_utils.project_points_by_matrix_torch(non_delay_lidar_np[:, :3],
                                                        transformation_matrix)
        non_delay_lidar_np = mask_points_by_range(non_delay_lidar_np,
                                        self.params['preprocess'][
                                            'cav_lidar_range'])
        non_delay_processed_lidar = self.pre_processor.preprocess(non_delay_lidar_np)
        ...
        selected_cav_processed.update(
            {'object_bbx_center': object_bbx_center[object_bbx_mask == 1],
             ...
             'non_delay_projected_lidar': non_delay_lidar_np,
             'non_delay_processed_features': non_delay_processed_lidar,
             'velocity': velocity})
    ...
    def collate_batch_train(self, batch):
        ...
        non_delay_processed_lidar_list = []
        ...
        for i in range(len(batch)):
            ...
            non_delay_processed_lidar_list.append(ego_dict['non_delay_processed_lidar'])
            ...
            if self.visualize:
                non_delay_lidar_np.append(ego_dict['non_delay_lidar_np'])
                lidar_np.append(ego_dict['lidar_np'])
        ...
        non_delay_merged_feature_dict = self.merge_features_to_dict(non_delay_processed_lidar_list)
        non_delay_processed_lidar_torch_dict = \
            self.pre_processor.collate_batch(non_delay_merged_feature_dict)
        ...
        output_dict['ego'].update({'object_bbx_center': object_bbx_center,
                                   ...
                                   'non_delay_processed_lidar': non_delay_processed_lidar_torch_dict,
        ...
        if self.visualize:
            non_delay_lidar_np = np.array(downsample_lidar_minimum(pcd_np_list=non_delay_lidar_np))
            non_delay_lidar_np = torch.from_numpy(non_delay_lidar_np)
            output_dict['ego'].update({'non_delay_lidar_np': non_delay_lidar_np})
            
            lidar_np = np.array(downsample_lidar_minimum(pcd_np_list=lidar_np))
            lidar_np = torch.from_numpy(lidar_np)
            output_dict['ego'].update({'lidar_np': lidar_np})
