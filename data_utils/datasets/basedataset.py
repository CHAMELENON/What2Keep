...
class BaseDataset(Dataset):
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        for cav_id, cav_content in scenario_database.items():
            ...
            # 有时延的雷达数据，使用timestamp定位
            data[cav_id]['lidar_np'] = pcd_utils.pcd_to_np(cav_content[timestamp_key_delay]['lidar'])

            # if self.visualize:
            timestamp_index_non_delay=timestamp_index
            timestamp_key_non_delay = self.return_timestamp_key(scenario_database, timestamp_index_non_delay)

            # 加入标准的，无时延的timestamp key
            data[cav_id]['non_delay_lidar_np'] = pcd_utils.pcd_to_np(cav_content[timestamp_key_non_delay]['lidar'])
    ...
    def time_delay_calculation(self, ego_flag):
        ...
        if self.async_mode == 'real':
            ...
            if self.train:
                time_delay = np.random.uniform(0, np.abs(self.async_overhead))
            else:
